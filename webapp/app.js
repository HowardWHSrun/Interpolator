...
async function buildManifest() {
  ...
  try {
    return await fetchJSON('/webapp/manifest.json');
  } catch {
    return {
      trains: [
        {
...
async function renderDirection(dirKey, dirSpec) {
  ...
  for (const r of rows) {
    ...
    const smin = Number(r.min_speed_mph) * 0.44704; // convert mph → m/s
    const smax = Number(r.max_speed_mph) * 0.44704; // convert mph → m/s
    const slike = Number(r.likely_speed_mph) * 0.44704; // convert mph → m/s
